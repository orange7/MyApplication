...
/**
 * Manages a given {@link rx.Observable}, handling activity destruction,
 * orientation changes, and posting to the UI thread. You construct one using
 * {@link me.tatarka.rxloader.RxLoaderManager#create(String, rx.Observable, RxLoaderObserver)}
 * .
 * 
 * @param <T>
...
public class RxLoader<T> extends BaseRxLoader<T> {
	...
	RxLoader(RxLoaderBackend manager, String tag, Observable<T> observable,
	...
	/**
	 * Starts the {@link rx.Observable} by subscribing to it. If the
	 * {@code Observable} is already started, then does nothing.
	 * 
	...
	/**
	 * Restarts the {@link rx.Observable} by subscribing to it, unsubscribing
	 * first if it has already been started.
	 * 
	 * @return the {@code RxLoader} for chaining
	 */
